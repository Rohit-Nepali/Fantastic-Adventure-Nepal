...
export default function PackageHero({ pkg }: { pkg: PackageDetail }) {
  const facts = [
    ...
    { label: "Transportation", value: pkg.transportation },
    ...
    <section>
      ...
      <div className="bg-white border-b border-[#eee] px-6 py-4">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-3">
          <a
            href="#inquiry"
            className="px-6 py-2.5 bg-[#00b5c4] text-white rounded-full text-sm font-medium hover:bg-[#009aaa] transition-colors"
          >
            Book Now
          </a>
          <Link
            href="/plan-your-trip"
            className="px-6 py-2.5 border border-[#ddd] text-[#333] rounded-full text-sm font-medium hover:border-[#00b5c4] hover:text-[#00b5c4] transition-colors"
          >
            Plan Your Trip
          </Link>
          <a
            ...
            className="px-6 py-2.5 border border-[#25d366] text-[#25d366] rounded-full text-sm font-medium hover:bg-[#25d366] hover:text-white transition-colors flex items-center gap-2"
          >
      ...
      {/* Trip Facts */}
      {facts.length > 0 && (
        <div className="bg-[#f8f6f2] border-b border-[#eee]">
          <div className="max-w-7xl mx-auto px-6 py-6">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {facts.map((fact) => (
                <div key={fact.label} className="text-center">
                  <p className="text-xs text-[#888] uppercase tracking-wide mb-1">{fact.label}</p>
                  <p className="text-sm font-semibold text-[#111]">{fact.value}</p>
                </div>
              ))}
            </div>
          </div>
