...
export default function WhyChooseUsSection({ data }: WhyChooseUsProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const elements = document.querySelectorAll(".why-choose-card");
      if (elements.length > 0 && cardsContainerRef.current) {
        gsap.fromTo(
          ".why-choose-card",
          { opacity: 0, y: 20 },
          {
      ...
      const lineElement = document.querySelectorAll(".why-choose-line");
      if (lineElement.length > 0 && headerRef.current) {
        gsap.fromTo(
          ".why-choose-line",
          ...
          {topRowFeatures.length > 0 && (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {topRowFeatures.map((feature) => (
                <article
                  key={`${feature?.number || Math.random()}-${feature?.title || ""}`}
